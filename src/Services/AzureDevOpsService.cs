...
/// <summary>
/// Service responsible for interacting with Azure DevOps REST APIs.
/// Handles authentication, request retries, and provides methods for accessing
/// projects, repositories, teams, and other Azure DevOps resources.
/// </summary>
public class AzureDevOpsService
{
    ...
    public AzureDevOpsService(HttpClient httpClient, string baseUrl, string personalAccessToken, string apiVersion = "7.1")
    {
    ...
    private async Task<T?> InvokeAsync<T>(string relativeUrl, HttpMethod method, object? body = null, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new Exception($"Azure DevOps API call failed after retries: {ex.Message}", ex);
        }
    }
    ...
    public async Task<List<Repository>> GetRepositoriesAsync(string projectName, string defaultBranch = "main", CancellationToken cancellationToken = default)
    {
        ...
        if (response?.Value != null)
        {
            foreach (var gitRepo in response.Value)
            {
                ...
                if (branches.Count > 0)
                {
                    gitRepo.DefaultBranch = branches.FirstOrDefault(b => b.IsDefault)?.Name;
                }
        ...
        try
        {
            ...
            if (tfvcResponse?.Value?.Count > 0)
            {
                ...
                repos.Add(new Repository
                {
                    ...
                    Name = projectName,
                    Url = $"{_baseUrl}/{projectName}/_apis/tfvc",
                    IsDisabled = false,
                    IsInMaintenance = false,
                    Size = 0,
                    RepositoryType = "tfvc",
                    DefaultBranch = await GetTfvcRootBranchAsync(projectName, cancellationToken),
                    Branches = await GetTfvcBranchesAsync(projectName, cancellationToken)
                });
            }
        }
        catch
    ...
    private async Task<List<Branch>> GetRepoBranchesAsync(string projectName, string repositoryName, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var branches = response?.Value?.Select(b => new Branch
            {
        ...
        {
            Logger.LogError($"Failed to get branches for repository {repositoryName}: {ex.Message}", ex);
            return new List<Branch>();
        }
    }

    public async Task<string> GetTfvcRootBranchAsync(string projectName, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var rootBranch = branches?.FirstOrDefault(b =>
                b.Name?.StartsWith($"$/{projectName}", StringComparison.OrdinalIgnoreCase) == true);

            return rootBranch?.Name ?? $"$/{projectName}";
        }
        catch (Exception ex)
        {
    ...
    public async Task<List<Branch>> GetTfvcBranchesAsync(string projectName, CancellationToken cancellationToken = default)
    {
        ...
        {
            Logger.LogInfo($"Getting TFVC root branch for project: {projectName}");
            var response = await InvokeAsync<ApiResponse<Branch>>($"_apis/tfvc/branches", HttpMethod.Get, cancellationToken: cancellationToken);
    ...
    public async Task<List<AdoTeamMember>> ExtractUsersAsync(string? projectNames = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                var project = await GetProjectAsync(projectName, includeTeams: true, includeTeamMembers: true, cancellationToken: cancellationToken);
                if (project != null)
                {
            ...
            {
                foreach (var team in project.Teams ?? Enumerable.Empty<AdoTeam>())
                {
