...
public class MigrationReport
{
    ...
    public async Task<string> SaveAsync(int workerCount)
    {
        ...
        var filename = $"{_timestamp}_{workerCount}workers_assessment.{extension}";
        ...
        string outputFile)
    {
        try
        {
            ...
            foreach (var project in projects)
            {
                ...
                report.AppendLine();
                report.AppendLine("| Status | Azure DevOps Repository | Type | GitHub Repository | GitHub URL | Error |");
                report.AppendLine("|--------|----------------------|------|------------------|------------|--------|");
                ...
                foreach (var repo in project.Repos)
                {
                    ...
                    report.AppendLine($"| {status} | {repo.Name} | {repo.RepositoryType.ToUpper()} | {repo.GitHubRepoName ?? "-"} | {githubUrl} | {errorMessage} |");
                }
                report.AppendLine();
                ...
                if (project.Teams.Any())
                {
                    ...
                    foreach (var team in project.Teams.Where(t => t.Members.Any()))
                    {
                        ...
                        report.AppendLine();
                        report.AppendLine("| Status | Member Email | GitHub Username | Error |");
                        report.AppendLine("|--------|--------------|----------------|--------|");
                        ...
                        foreach (var member in team.Members)
                        {
                            ...
                            report.AppendLine($"| {status} | {member.UniqueName} | {member.GitHubUserName ?? "-"} | {errorMessage} |");
                        }
                        report.AppendLine();
                    }
                }
            ...
            Logger.LogSuccess($"Report saved to: {Common.CreateClickableMarkdownLink(outputFile, $"file://{outputFile}")}");
        }
        catch (Exception ex)
        {
        ...
        string outputFile = "migration_report.md")
    {
        ...
        // Save report to file
        File.WriteAllText(outputFile, report.ToString(), Encoding.UTF8);
        //Console.WriteLine($"Migration report saved to: {outputFile}");
        Logger.LogSuccess($"\nReport saved to: {Common.CreateClickableMarkdownLink(outputFile, $"file://{outputFile}")}");
    }
}